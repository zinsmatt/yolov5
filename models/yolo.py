...
class Detect(nn.Module):
    ...
    def forward(self, x):
        # print()
        z = []  # inference output
        # print("INPUT = ", x[0].shape, x[1].shape, x[2].shape)
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                # print("y.vieeeewwwwwww = ", y.view(bs, -1, self.no).shape)
        ...
        #     print("RET shape = ", len(ret), ret[0].shape, ret[1].shape, ret[2].shape)
        # else:
        #     print(ret[0].shape, " ||||||| ", ret[1][0].shape, ret[1][1].shape, ret[1][2].shape)
        # print("RET = ", len(ret), ret[0].shape)
        return ret

    def _make_grid(self, nx=20, ny=20, i=0):
        # print("make grid ", nx, " ", ny )
...
class Model(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        # cv2.imshow("fen", img[:, :, ::-1])
        # cv2.waitKey()
        # print("-"*100 + " FORWARD ONCE !!!!!!!!!!!!!")
        # print(x.shape)
        y, dt = [], []  # outputs
        for m in self.model:
